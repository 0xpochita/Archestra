import { z } from "zod";
import { TOKEN_IDS } from "@/lib/chain/tokens";

export const SPENDER_KINDS = [
  "deposit",
  "swap",
  "yield",
  "harvest",
...
const swapConfigSchema = z
  ...
  .refine((config) => config.tokenIn !== config.tokenOut, {
    message: "the two tokens must differ",
    path: ["tokenOut"],
  });
...
const conditionConfigSchema = z.object({
  ...
  comparator: z.union([
    z.literal(COMPARATOR_BELOW),
    z.literal(COMPARATOR_ABOVE),
  ]),
...
export type StepConfigOf<Kind extends StepConfig["kind"]> = Extract<
  StepConfig,
  { kind: Kind }
>;
