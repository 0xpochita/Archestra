...
- Decision (2026-07-29), the user session layer: a live resolved executor pointer over standing vault authority repeats the exact shape drained at Furucombo (2021, 14M), Socket (2024, 3.3M) and Li.Fi (2024, over 10M). Therefore: the registry only publishes executor candidates, each vault obeys the executor its owner accepted (bootstrap consent is the owner's own create transaction), and every grant is clamped by an owner set session (per run cap, per day cap, expiry, one transaction revocation). Rejected here: per run user signatures (break Chainlink triggered runs), opt out auto adoption windows (a dormant user is still drained, fails the stated goal), and sign on next run acceptance as the default consent (blind signing risk, Bybit 2025; an explicit accept transaction is the default, signature acceptance may be added as an opt in convenience later).
